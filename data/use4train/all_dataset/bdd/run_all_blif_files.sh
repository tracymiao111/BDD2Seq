...
# Function to extract the variable ordering from the output file
extract_variable_ordering() {
    grep -A 36 "Current variable ordering in BDD" "$1" | grep "Position" | awk '{print $NF}' | paste -sd "," - | sed 's/,/, /g'
}
...
find "$BLIF_FOLDER" -type f -name "*.blif" | while read -r BLIF_FILE; do
    ...
    # Set the output directory based on the circuit name and parent directory
    OUTPUT_DIR="$(dirname "$BLIF_FILE")/${CIRCUIT_NAME}_BDD_all"
    ...
    for ALGO in "${ALGORITHMS[@]}"; do
        ...
        # Define the output file for this algorithm in the same folder as the .blif file
        OUTPUT_FILE="${OUTPUT_DIR}/${algo_name}_output.txt"

        # Run the program with a time limit of 20 minutes and capture output
        timeout 4m ./optimalAlgorithm "$BLIF_FILE" "$algo_value" > "$OUTPUT_FILE" 2>&1

        # Check if the program timed out
        if [ $? -eq 124 ]; then
            echo "Algorithm $algo_name timed out after 8 minutes." | tee -a "$OUTPUT_FILE"
            continue
        else
            echo "Completed: $algo_name." | tee -a "$OUTPUT_FILE"
        fi
        ...
        # Extract the total DAG size from the output
        dag_size=$(grep "Total number of unique BDD nodes" "$OUTPUT_FILE" | awk '{print $NF}')
        ...
        if [ ! -z "$dag_size" ]; then
            if [ -z "$smallest_dag_size" ] || [ "$dag_size" -lt "$smallest_dag_size" ]; then
                ...
                best_algorithm=$algo_name
                best_algorithm_output="$OUTPUT_FILE"
                best_variable_ordering=$(extract_variable_ordering "$OUTPUT_FILE")
            fi
        fi
    ...
    # Output the best algorithm and variable ordering to a text file in the same folder
    if [ ! -z "$best_algorithm" ]; then
        echo "The algorithm that found the smallest BDD size for $CIRCUIT_NAME: $best_algorithm"
        echo "Smallest BDD size: $smallest_dag_size"
        echo "Best algorithm: $best_algorithm" > "${OUTPUT_DIR}/best_algorithm.txt"
        echo "Smallest BDD size: $smallest_dag_size" >> "${OUTPUT_DIR}/best_algorithm.txt"
        echo "Variable ordering: { $best_variable_ordering }" >> "${OUTPUT_DIR}/best_algorithm.txt"
        echo "Full output is available in $best_algorithm_output" >> "${OUTPUT_DIR}/best_algorithm.txt"
    else
        echo "No valid algorithms found without errors for $CIRCUIT_NAME."
        echo "No valid algorithms found without errors." > "${OUTPUT_DIR}/best_algorithm.txt"
