...
class BlifDataset(Dataset):
    def __init__(self, pt_file):
        ...
        if isinstance(data, dict):
            if 'graphs' in data and 'labels' in data and 'circuit_names' in data:
            ...
            elif 'graphs' in data and 'labels' in data:
                self.graphs = data['graphs']
                self.rank_wise_labels = data['labels']
                self.circuit_names = [f"circuit_{i}" for i in range(len(self.graphs))]  # Default names
                print("[WARNING] 'circuit_names' not found in data. Using default circuit names.")
            else:
                raise KeyError("The .pt file must contain 'graphs', 'labels', and optionally 'circuit_names' keys.")
        else:
            raise ValueError("Unsupported data format in .pt file. Expected a dictionary with 'graphs', 'labels', and optionally 'circuit_names' keys.")
        
        if len(self.graphs) != len(self.rank_wise_labels) or len(self.graphs) != len(self.circuit_names):
            raise ValueError("The lengths of 'graphs', 'labels', and 'circuit_names' must be equal.")
