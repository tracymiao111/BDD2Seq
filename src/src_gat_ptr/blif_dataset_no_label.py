# blif_dataset_infer_no_label.py
...
class BlifDataset(Dataset):
    def __init__(self, pt_file):
        ...
        if isinstance(data, dict):
            if 'graphs' in data and 'circuit_names' in data:
                self.graphs = data['graphs']  # List of DGLGraph objects
                self.circuit_names = data['circuit_names']  # List of circuit names

                # Check if 'labels' are present
                if 'labels' in data:
                    self.rank_wise_labels = data['labels']  # List of lists or tensors
                    if len(self.graphs) != len(self.rank_wise_labels):
                        raise ValueError("The lengths of 'graphs' and 'labels' must be equal.")
                else:
                    self.rank_wise_labels = None  # Labels are not available
                    print("[INFO] 'labels' not found in data. Proceeding without labels.")
            else:
                raise KeyError("The .pt file must contain 'graphs' and 'circuit_names' keys.")
        else:
            raise ValueError("Unsupported data format in .pt file. Expected a dictionary with 'graphs' and 'circuit_names' keys.")
        
        if len(self.graphs) != len(self.circuit_names):
            raise ValueError("The lengths of 'graphs' and 'circuit_names' must be equal.")
        
        print(f"Number of graphs: {len(self.graphs)}")
        print(f"Number of circuit names: {len(self.circuit_names)}")
    ...
    def __getitem__(self, idx):
        ...
        circuit_name = self.circuit_names[idx]
        ...
        # If labels are available, process them
        if self.rank_wise_labels is not None:
            ...
            return graph, valid_node_indices, circuit_name
        else:
            # No labels available, return graph and circuit name only
            return graph, circuit_name